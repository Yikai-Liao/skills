...
当前运行时只保留核心约束，已移除通用写作教程、完整源码示例和大部分教学案例。下文关于“运行时摘录、表格、案例”的说明记录历次采用形态，不表示当前仍加载这些内容；完整案例版本可查 Git 提交 `85985f7`，既有验收场景仍保存在测试文件中。来源核对记录保留，避免重新考证或将历史结果误算为新版证据。
...
会话输出与设计讨论合并时，用户反馈指出：先前的调研汇报和重试设计示例，虽已缩短、分段，仍要求读者理解临时引入的系统与多项抽象关系。按用户要求，共同指导扩展到非持久化的会话回复与持久化文本，设计讨论纳入按需参考。
...
早期版本的例子采用双方已讨论的流程精简任务，说明如何把抽象概括展开为实际动作；它是教学改写，不是已经过真人对照验证的最佳版本。没有从研究导出字数、短句、分层或类比配额，也不根据阅读困难推断用户的诊断或知识水平。

后续用户提供旧版技能产出的会议纪要，指出段落各自通顺，但全篇反复换观察角度、重启目标，父子标题混入不同职责的议题。由此补充文章主线、章节承接、同级与父子关系的明确约束，并保留独立 FAQ 的适用边界。该反馈用于定位组织问题，不作为会议事实核验；测试采用自拟材料，不收录原纪要的业务内容。
...
此前“本技能只管表达”的范围在合并时扩展为：共同规则覆盖面向读者的输出，设计协作是条件性分支。普通润色与翻译仍保持既定流程的含义；工具执行、验证和审批权限继续来自任务与项目要求。运行时不依赖或调用其他技能。
...
| [Nordmeyer 与 Frank：The pragmatics of negation across contexts](https://langcog.stanford.edu/papers/NF-cogsci2015.pdf) | 完整读取六页；两项实验分析 92、184 名英语母语成人的情境合宜度判断。支持性情境提高真实否定句评分；早期版本的摘要是简化中文摘述，未复跑数据，也未推广成句式禁令 |
...
| Wagtail v6.4.1：[首页源码](https://github.com/wagtail/wagtail/blob/v6.4.1/docs/index.rst)、[首个站点教程](https://docs.wagtail.org/en/v6.4.1/getting_started/tutorial.html) | 核对首页完整分组及代表教程的环境、管理员和启动步骤；早期版本采用中文概括，未安装 Wagtail，也未逐页审计文档站点 |
...
| [Firecracker Getting Started](https://github.com/firecracker-microvm/firecracker/blob/dcfc69b625d0ffd9c65efd81fe2eff445012881a/docs/getting-started.md) | Linux/KVM、设备访问和演示/生产隔离边界；早期版本采用中文摘要，未启动虚拟机 |
...
- Fowler 的 [Extract Variable](https://refactoring.com/catalog/extractVariable.html) 与 [Split Phase](https://refactoring.com/catalog/splitPhase.html)：只读公开目录示例。早期版本的价格公式为教学改写，非真实项目重构历史。
...
早期版本仅摘述上述 API 名称、短签名与行为边界；布尔、单位和 JSON 兼容例子是自拟教学材料。没有从指南或源码存在这些名称推出已测得可读性收益。
...
- Rust 官方书[所有权章节](https://doc.rust-lang.org/book/ch04-01-what-is-ownership.html)与[固定 E0382 输出](https://github.com/rust-lang/book/blob/1500248d8f230566e4ec9f27fcbb8fe9e2898ab1/listings/ch04-understanding-ownership/no-listing-04-cant-use-after-move/output.txt)：早期版本的两个短节选匹配该输出，未实际运行 Rust 编译器；克隆建议的性能条件保留。
...
- Django 5.2 的[文档入口](https://docs.djangoproject.com/en/5.2/#how-the-documentation-is-organized)和[固定目录源码](https://github.com/django/django/blob/9e7cc2b628fe8fd3895986af9b7fc9525034c1b0/docs/index.txt#L49-L69)：真实的教程、概念、参考与任务入口，早期版本的示例为中文概括；不要求小手册凑齐四类。
- IPCC AR6 [Summary for Policymakers](https://www.ipcc.ch/report/ar6/syr/summary-for-policymakers/)，2023 年 A.1/A.1.1：早期版本只摘述 2011—2020 相对 1850—1900 高约 1.1°C 的含义，未核验底层气候数据。
...
- [RFC 2119](https://www.rfc-editor.org/rfc/rfc2119.txt) 第 1、3、5 节与 [RFC 8174](https://www.rfc-editor.org/rfc/rfc8174.txt) 第 2 节：早期版本给出中文释义，保留规范强度、大写适用条件及不用大写词仍可能有规范要求的边界。
