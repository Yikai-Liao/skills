...
When source acquisition is needed, resolve identifiers to authoritative records, persist every attempted and successful retrieval, and validate downloaded content before treating it as a paper or supplement. If the `huggingface-papers` skill is available in the current environment, use it before arXiv for both paper discovery and obtaining a specified paper's readable Markdown; fall back to the arXiv route only when that skill is unavailable, cannot complete the requested operation, or does not cover the paper. For that fallback, prefer a cleaned Markdown reading copy derived from cached official arXiv HTML; do not read raw HTML by default. Retain the HTML for provenance and structural checks, and fall back without repeated probing when HTML is unavailable.
...
- When delegating survey work, require each subagent to write its assigned Markdown artifact before replying. Treat a chat response only as a receipt containing the artifact path and blockers; never treat an oral summary as a deliverable or merge source. Follow the artifact-first protocol in [collaboration.md](references/collaboration.md).
